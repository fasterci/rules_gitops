...
def _gcloud_run_impl(ctx):
    ...
    if "{" in ctx.attr.service:
        service = stamp(ctx, service, files, ctx.label.name + ".service-name", True)
    ...
    for inattr in ctx.attr.srcs:
        for infile in inattr.files.to_list():
            if command == "replace":
                statements += "{template_engine} --template={infile} --variable=PROJECT=\"$PROJECT\" --variable=REGION=\"$REGION\" --variable=SERVICE=\"$SERVICE\" --stamp_info_file={info_file} | gcloud run services replace - --project=\"$PROJECT\" --region=\"$REGION\"\n".format(
                ...
                statements += "gcloud run services delete \"$SERVICE\" --project=\"$PROJECT\" --region=\"$REGION\" --quiet\n"
            else:
...
{}
    ...
    if "{" in service:
        fail("unable to gitops service with placeholders %s" % service)
    if "{" in region:
...
"""
    ...
    outputs = []
    script_template = "{template_engine} --template={infile} --variable=PROJECT={project} --variable=REGION={region} --variable=SERVICE={service} --stamp_info_file={info_file}\n"
...
def cloudrun_deploy(
    ...
    for reservedname in ["PROJECT", "REGION", "SERVICE"]:
    ...
    if not gitops:
        ...
        if not service:
            fail("service must be defined for gitops cloudrun_deploy")
        if not region:
